...
def solution(node, idx):
    ...
    pre_link = None
    link = node
    future_link = link.next_item

    while index_item != idx:
        pre_index = index_item
        index_item = pre_index + 1
        pre_link = link
        link = link.next_item
        future_link = future_link.next_item
    if 0 == idx:
        return node.next_item
    pre_link.next_item = future_link
    return node
